# 🕐 ClockW3 - Архитектура и система координат

## 📁 Структура проекта

### Организация кода

Проект использует **PBXFileSystemSynchronized** (Xcode 16+) для автоматической синхронизации файловой системы с проектом.

```
ClockW3/
├── Shared/              ← Код общий для приложения и виджета
│   ├── Models/          ← Модели данных (WorldCity, ClockConstants)
│   ├── ViewModels/      ← Логика представления (ClockViewModel)
│   ├── Views/           ← SwiftUI Views (ClockFaceView, CityArrowsView, etc.)
│   └── Helpers/         ← Утилиты (AngleCalculations, SharedUserDefaults, etc.)
├── ClockW3/             ← Код только для основного приложения
│   ├── ContentView.swift
│   ├── SwiftUIClockApp.swift
│   └── Helpers/         ← App-specific helpers (ClockHaptics)
└── ClockW3Widget/       ← Код только для виджета
    └── ClockW3Widget.swift
```

### Зачем Shared?

**Проблема:** Раньше общие файлы жили в `ClockW3/` и требовали ручного добавления в Widget Extension через `membershipExceptions` в project.pbxproj.

**Решение:** Теперь все общие файлы в `Shared/` - автоматически доступны обоим таргетам без ручной настройки.

### Как добавить новый файл?

**Для общего кода (используется и в app, и в widget):**
- Создайте файл в `Shared/` в соответствующей папке
- Файл автоматически будет доступен обоим таргетам

**Для app-specific кода:**
- Создайте файл в `ClockW3/`
- Будет доступен только основному приложению

**Для widget-specific кода:**
- Создайте файл в `ClockW3Widget/`
- Будет доступен только виджету

### App Groups для синхронизации данных

Приложение и виджет используют **App Group** для шаринга настроек (выбранные города):

```swift
// Shared/Helpers/SharedUserDefaults.swift
static let appGroupID = "group.exrector.ClockW3"
```

**Важно:** Оба таргета должны иметь App Group в Signing & Capabilities:
- ClockW3: ✓ group.exrector.ClockW3
- ClockW3WidgetExtension: ✓ group.exrector.ClockW3

### Trade-offs и ограничения

**Плюсы PBXFileSystemSynchronized:**
- ✅ Автоматическая синхронизация файлов
- ✅ Нет конфликтов в project.pbxproj при git merge
- ✅ Простое добавление файлов

**Минусы:**
- ❌ Требует Xcode 16.1+
- ❌ Все файлы в директории автоматически включаются в таргет
- ❌ CI/CD должен поддерживать Xcode 16.1+
...
**Важно:** Углы **ОТРИЦАТЕЛЬНЫЕ** из-за `-CGFloat(degrees)`:
```swift
